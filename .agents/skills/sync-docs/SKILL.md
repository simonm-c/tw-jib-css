...
| `tw-jib-css/src/automatic-contrast/_index.css`                                    | `automatic-contrast.md`                | `tw-jib-css/automatic-contrast`       |
| `tw-jib-css/src/ripple/_index.css`                                                | `ripple.md`                            | `tw-jib-css/ripple`                   |
...
| `tw-jib-css-experimental/src/{color-transforms,automatic-contrast}/_override.css` | `functions.md`                         | `tw-jib-css-experimental/functions`   |
...
`core/` and `contrast/` publish nothing. The suites that need them share them,
import them.
...
- **Whether it has a wildcard** (`*`), which means it accepts values
...
### 2f. Color spaces section (color transforms only)

- Section exists if utility supports color space modifiers
- Shows comparison across at least the default (oklch) + 3 alternatives
- Links to the Color Spaces guide: `/guide/color-spaces`
...
- **Layout**: grid
- **Experimental**: corner, interpolate, picker, functions, wcag-badge
