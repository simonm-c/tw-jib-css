...
const overrides = ['bg-lightness-*', 'bg-saturation-*', 'bg-hue-rotate-*', 'text-contrast-*'];
</script>
    ...
    <div class="mx-auto w-full max-w-[1300px] space-y-10">
      <div class="space-y-3 max-w-[60ch]">
        ...
        <p class="text-lg leading-relaxed opacity-85">
          Import the package root and you get both. Only one can change code you have already
          written, which is why they are separable.
        </p>
      </div>
      ...
      <div class="grid md:grid-cols-2 gap-6">
        ...
        <div class="rounded-2xl border border-emerald-500/40 bg-emerald-500/5 p-6 space-y-4">
          ...
          <p class="text-sm leading-relaxed opacity-80">
            Classes with no stable equivalent. Nothing you already use has these names, so nothing
        ...
        <div class="rounded-2xl border border-amber-500/40 bg-amber-500/5 p-6 space-y-4">
          ...
          <p class="text-sm leading-relaxed opacity-80">
            Classes the stable package already defines, replaced. Same names, same results where
            they overlap, but the functions behind them also reach into a gradient stop or a layered
            shadow. Take this half for the reach, and take it deliberately, because it changes pages
          ...
          <!-- mt-4 explicitly. The parent's space-y-4 puts margin-block-end on
               ...
               on the <ul> above beats it and leaves this box flush against the
               last list row, measured at 0px. -->
          <div
            class="mt-4 rounded-lg bg-gray-900 dark:bg-gray-800 px-3 py-2 font-mono text-[11px] text-gray-300"
          >
      ...
      <div class="grid md:grid-cols-2 gap-6 items-center">
        ...
        <p class="text-sm opacity-70 leading-relaxed m-0">
          Experimental always comes <strong class="font-bold opacity-100">after</strong> stable.
          Wrong order, and the overrides do nothing.
        </p>
      </div>
    </div>
  </section>
