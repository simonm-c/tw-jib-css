...
| Call                                | Returns                                                 |
| ----------------------------------- | ------------------------------------------------------- |
| `--tw-jib--wcag-rating(<bg>, <fg>)` | `"AAA"`, `"AA"`, `"AA Large"` or `"Fail"` for that pair |
...
<Example stretch>
  <div class="grid grid-cols-2 gap-3 p-6">
    <div class="relative rounded-lg bg-teal-500 p-6 text-center [color:--tw-jib--auto-contrast(var(--color-teal-500),aaa)] after:absolute after:right-2 after:top-2 after:rounded after:bg-black/70 after:px-1.5 after:py-0.5 after:font-mono after:text-[10px] after:font-bold after:text-white after:content-[--tw-jib--wcag-rating(var(--color-teal-500),--tw-jib--auto-contrast(var(--color-teal-500),aaa))]">
      <span class="font-bold">solved for aaa</span>
    </div>
    <div class="relative rounded-lg bg-teal-500 p-6 text-center text-white after:absolute after:right-2 after:top-2 after:rounded after:bg-black/70 after:px-1.5 after:py-0.5 after:font-mono after:text-[10px] after:font-bold after:text-white after:content-[--tw-jib--wcag-rating(var(--color-teal-500),white)]">
...
<div
  class="bg-teal-500 text-white
         after:content-[--tw-jib--wcag-rating(var(--color-teal-500),white)]"
>
  plain white
</div>
```

The two tiles carry the same background. The left one takes its text color from [`--tw-jib--auto-contrast()`](/guide/automatic-contrast) and the badge confirms the level it asked for; the right one uses plain white and the badge reports what that actually scores. Nothing is hard-coded in either label.
...
That tolerance is not cosmetic. [`--tw-jib--auto-contrast()`](/guide/automatic-contrast) hits its ratio _exactly_, which produces ties by design. Without the epsilon this function would contradict the solve sitting next to it on the same element.
...
`--tw-jib--wcag-rating()` ships in the `functions` module. The badge utility is a separate module and can be taken on its own.
