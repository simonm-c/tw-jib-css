...
function apply(value: string, name: string) {
  ...
  active.value = name;
  try {
    localStorage.setItem(STORAGE_KEY, JSON.stringify({ name, value }));
  } catch {}
}
...
<template>
  <div class="jib-brandpicker flex items-center gap-2 px-2" title="Trim the sail">
    ...
    <div class="flex items-center gap-1">
      <button
        ...
        :class="[
          preset.css,
          'size-4 rounded-full border-0 cursor-pointer shrink-0 transition-shadow bg-ripple ripple-color-white/40 ripple-position-center',
          active === preset.name
      ...
      <label
        :class="[
          'relative size-4 overflow-hidden rounded-full border border-jib-wind/40 cursor-pointer shrink-0',
          active === '__custom'
            ? 'ring-2 ring-[var(--color-jib-wind)] ring-offset-1 ring-offset-gray-900'
            : '',
        ]"
