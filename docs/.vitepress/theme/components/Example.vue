...
<template>
  ...
  >
    <p
      v-if="label"
      class="flex items-center gap-2 border-b border-jib-wind/25 bg-jib-brand -bg-saturation-60 bg-lighten-95 dark:bg-darken-88 px-4 py-2 font-mono text-[11px] uppercase tracking-[0.2em] text-a11y-aaa"
    >
    ...
    <div class="border-t border-jib-wind/25 bg-jib-brand -bg-saturation-60 bg-lighten-95 dark:bg-darken-88">
      <button
        class="w-full border-0 bg-transparent bg-ripple ripple-color-jib-wind/40 ripple-position-center px-4 py-2 text-left font-mono text-[11px] uppercase tracking-[0.2em] cursor-pointer text-a11y-aaa transition-colors"
        @click="showCode = !showCode"
      >
        <span aria-hidden="true" class="mr-2">⌘</span>{{ showCode ? 'hide code' : 'show code' }}
      </button>
      <div v-show="showCode" class="border-t border-jib-wind/20">
        <pre class="m-0 overflow-x-auto bg-jib-brand -bg-saturation-40 bg-darken-80 dark:bg-darken-88 p-4 text-[13px] leading-relaxed text-a11y-aa"><code>{{ code }}</code></pre>
      </div>
    </div>
  </div>
</template>
