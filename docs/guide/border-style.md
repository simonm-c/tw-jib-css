...
<!-- llm-context: Sets border-style one side at a time, which stock TailwindCSS v4 cannot do. Covers groove, ridge, inset and outset, in physical, axis and logical side variants. -->
...
Plain `border-*-style` declarations. No relative color syntax, no `@property`
...
The logical variants (`s` and `e`) follow writing direction. In LTR layouts `s` is left and `e` is right; in RTL they swap.
