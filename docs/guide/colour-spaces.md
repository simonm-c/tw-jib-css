...
**HSL** and **RGB** are _not_ perceptually uniform. The same numeric lightness change on different hues produces visibly different results. HSL's "50% lightness" looks much brighter on yellow than on blue. These spaces are still useful when you want direct control over specific channels, but they are less predictable for design-system-level colour manipulation.
