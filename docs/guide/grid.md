...
<!-- llm-context: Named grid-template-areas and grid-area placement, which TailwindCSS v4 leaves out. Tailwind ships grid-cols and grid-rows but no grid-template-areas at all. -->
...
Plain `grid-template-areas` and `grid-area`. No relative color syntax, no
...
read no color. Taking it costs you nothing but the two utilities below. See [take
