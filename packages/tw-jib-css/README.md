...
<p align="center"><strong>tw-jib-css</strong>. WCAG-exact text contrast, gradient borders and colour transforms, on every engine.</p>
...
```html
<span class="text-contrast-aa-plus">exactly 5:1</span>
```
...
`text-contrast-*` reads the composed result. So this does what it looks like it does:

```html
<div class="bg-red-500 bg-hue-rotate-120 -bg-saturation-30 bg-lightness-25">
  <span class="text-contrast-aa">contrasted against the transformed background</span>
</div>
```
...
**Lightness, saturation and `text-contrast-*`** cover 17 spaces:
...
**Hue rotate** covers the same list minus `color-mix`, so 16. `color-mix()` blends toward a second
colour, and a rotation has none to name: lightness aims at white or black and saturation aims at
grey, but the hue a rotation lands on is the result you are asking for.
...
Importing several modules costs no more than importing the root entry. The root composes from the
partials so the shared pipeline lands once rather than once per module. The test suite covers both
halves: that each module resolves every `var()` it emits alone, and that the combined entry doesn't
re-emit the shared core.
...
eight interpolation modes (`srgb` `hsl` `oklab` `oklch` `longer` `shorter` `increasing` `decreasing`) and
an animated spin.
...
> `bg-hue-rotate-*` and `text-contrast-*` onto CSS `@function`. Same names, same output, a different
> code path.
>
