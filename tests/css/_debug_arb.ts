...
async function main() {
  ...
  let css = await compile('bg-[--tw-jib--lightness(var(--color-blue-500),60)]', { experimental: true });
  ...
  css = await compile('bg-linear-to-r from-[--tw-jib--lightness(var(--color-blue-500),60)] to-blue-500', { experimental: true });
  util = css.split('\n').filter(l => l.includes('lightness') || l.includes('gradient') || l.includes('from'));
  console.log('\n=== from-[fn] ===');
  console.log(util.join('\n').slice(0, 500) || '(no matches)');
  
  // What about from with underscores?
  css = await compile('bg-linear-to-r from-[--tw-jib--lightness(var(--color-blue-500)_60)] to-blue-500', { experimental: true });
