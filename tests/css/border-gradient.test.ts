  ...
  ({ compile }) => {
    ...
    test('from/via/to stops', async () => {
      const css = await compile(
        'bg-linear-to-r from-pink-500 via-yellow-400 to-cyan-500 border-linear-to-b border-from-rose-500 border-to-cyan-500',
      );
    ...
    test('gradient stop positions', async () => {
      const css = await compile(
        `bg-linear-to-r from-blue-500 from-10% via-purple-500 via-30% to-pink-500 to-90% ${BORDER}`,
      );
...
describe.each(suiteScenarios('border-gradient'))(
  ...
  ({ compile }) => {
    test.each([
      ...
      const css = await compile(
        `${cls} from-red-500 to-blue-500 border-linear-to-b border-from-amber-400 border-to-emerald-500`,
      );
...
describe.each(suiteScenarios('border-gradient'))(
  ...
  ({ compile }) => {
    test('border-linear-45', async () => {
      const css = await compile(
        'bg-slate-800 border-linear-45 border-from-rose-500 border-to-cyan-500',
      );
    ...
    test('border-linear-90', async () => {
      const css = await compile(
        'bg-slate-800 border-linear-90 border-from-rose-500 border-to-cyan-500',
      );
    ...
    test('border-linear-135', async () => {
      const css = await compile(
        'bg-slate-800 border-linear-135 border-from-rose-500 border-to-cyan-500',
      );
    ...
    test('border-linear-[137deg]', async () => {
      const css = await compile(
        'bg-slate-800 border-linear-[137deg] border-from-rose-500 border-to-cyan-500',
      );
      expect(css).toContain('137deg');
      expect(css).toContain('linear-gradient');
      expect(css).toContain(BG_LAYER);
...
describe.each(suiteScenarios('border-gradient'))('border gradient radial, $name', ({ compile }) => {
  ...
  test('border-radial-[at_25%_25%]', async () => {
    const css = await compile(
      'bg-slate-800 border-radial-[at_25%_25%] border-from-rose-500 border-to-cyan-500',
    );
    expect(css).toContain('radial-gradient');
    expect(css).toContain('border-color: transparent');
    expect(css).toContain(BG_LAYER);
  ...
  test('border-radial-[at_top]', async () => {
    const css = await compile(
      'bg-slate-800 border-radial-[at_top] border-from-rose-500 border-to-cyan-500',
    );
    expect(css).toContain('radial-gradient');
    expect(css).toContain(BG_LAYER);
  });
...
describe.each(suiteScenarios('border-gradient'))('border gradient conic, $name', ({ compile }) => {
  ...
  test('border-conic-45', async () => {
    const css = await compile(
      'bg-slate-800 border-conic-45 border-from-rose-500 border-via-yellow-400 border-to-cyan-500',
    );
    expect(css).toContain('45 * 1deg');
    expect(css).toContain('conic-gradient');
    expect(css).toContain(BG_LAYER);
  ...
  test('border-conic-90', async () => {
    const css = await compile(
      'bg-slate-800 border-conic-90 border-from-rose-500 border-via-yellow-400 border-to-cyan-500',
    );
    expect(css).toContain('90 * 1deg');
    expect(css).toContain(BG_LAYER);
  });
  ...
  test('border-conic-180', async () => {
    const css = await compile(
      'bg-slate-800 border-conic-180 border-from-rose-500 border-via-yellow-400 border-to-cyan-500',
    );
...
describe.each(suiteScenarios('border-gradient'))('border spin, $name', ({ compile }) => {
  ...
  test('the animation shorthand reads the direction, so a later utility can turn the spin round', async () => {
    ...
    expect(css).toMatch(
      /animation: border-spin var\(--tw-jib--border-spin-duration\)[^;]*var\(--tw-jib--border-spin-direction\)/,
    );
  });
  ...
  test('border-spin-reverse sets the direction without redeclaring the animation', async () => {
    ...
    expect(css).toContain('--tw-jib--border-spin-direction: reverse');
    expect(css).toMatch(
      /\.border-spin-reverse \{\s*--tw-jib--border-spin-direction: reverse;\s*\}/,
    );
    expect(css).toContain(BG_LAYER);
  });
  ...
  test('border-spin-duration accepts a theme name, so a project can name its own speeds', async () => {
    const css = await compile('border-conic-0 border-spin border-spin-duration-slow', {
      extra: '@theme { --tw-jib--border-spin-duration-slow: 4s; }',
    });
    expect(css).toContain(
      '--tw-jib--border-spin-duration: var(--tw-jib--border-spin-duration-slow)',
    );
  });
  ...
  test('border-spin-duration accepts a bare custom property, so the duration can be driven at runtime', async () => {
    ...
    expect(css).toContain('--tw-jib--border-spin-duration: var(--spin-speed)');
  });
  ...
  /* Nesting `&.border-spin` would put the bare class in every variant's
   * selector, so motion-safe:border-spin could only match an element already
   * carrying border-spin. Spin uses cascade origin instead. */
  test('a variant of border-spin does not demand the class it replaces', async () => {
    const css = await compile(
      'bg-slate-800 border-conic-0 motion-safe:border-spin border-from-rose-500 border-to-cyan-500',
    );
    expect(css).toContain('prefers-reduced-motion: no-preference');
    expect(css).toContain('animation: border-spin');
    expect(css).not.toMatch(/&\.border-spin/);
  });
});
...
describe.each(suiteScenarios('border-gradient'))(
  ...
  ({ compile }) => {
    test('both /srgb', async () => {
      const css = await compile(
        'bg-linear-to-r/srgb from-red-500 to-blue-500 border-linear/srgb border-linear-to-b border-from-amber-400 border-to-emerald-500',
      );
    ...
    test('both /oklch', async () => {
      const css = await compile(
        'bg-linear-to-r/oklch from-red-500 to-blue-500 border-linear/oklch border-linear-to-b border-from-amber-400 border-to-emerald-500',
      );
    ...
    test('bg/srgb + border/longer', async () => {
      const css = await compile(
        'bg-linear-to-r/srgb from-red-500 to-blue-500 border-linear/longer border-linear-to-b border-from-amber-400 border-to-emerald-500',
      );
      expect(css).toContain('in srgb');
    ...
    test('bg/longer + border/srgb', async () => {
      const css = await compile(
        'bg-linear-to-r/longer from-red-500 to-blue-500 border-linear/srgb border-linear-to-b border-from-amber-400 border-to-emerald-500',
      );
    ...
    test('bg/hsl + border/oklch', async () => {
      const css = await compile(
        'bg-linear-to-r/hsl from-red-500 to-blue-500 border-linear/oklch border-linear-to-b border-from-amber-400 border-to-emerald-500',
      );
    ...
    test('bg/oklch + border/hsl', async () => {
      const css = await compile(
        'bg-linear-to-r/oklch from-red-500 to-blue-500 border-linear/hsl border-linear-to-b border-from-amber-400 border-to-emerald-500',
      );
      expect(css).toContain('in oklch');
      expect(css).toContain('in hsl');
      expect(css).toContain(BG_LAYER);
    ...
    test('bg/increasing + border/decreasing', async () => {
      const css = await compile(
        'bg-linear-to-r/increasing from-red-500 to-blue-500 border-linear/decreasing border-linear-to-b border-from-amber-400 border-to-emerald-500',
      );
    ...
    test('bg/decreasing + border/increasing', async () => {
      const css = await compile(
        'bg-linear-to-r/decreasing from-red-500 to-blue-500 border-linear/increasing border-linear-to-b border-from-amber-400 border-to-emerald-500',
      );
      expect(css).toContain('decreasing');
      expect(css).toContain('increasing');
      expect(css).toContain(BG_LAYER);
...
describe.each(suiteScenarios('border-gradient'))(
  ...
  ({ compile }) => {
    test('bg-radial + border-linear', async () => {
      const css = await compile(
        'bg-radial from-blue-500 to-purple-900 border-linear-to-r border-from-amber-400 border-to-emerald-500',
      );
      expect(css).toContain('radial-gradient');
      expect(css).toContain('linear-gradient');
      expect(css).toContain(BG_LAYER);
    ...
    test('bg-radial + border-conic', async () => {
      const css = await compile(
        'bg-radial from-blue-500 to-purple-900 border-conic-0 border-from-amber-400 border-via-rose-500 border-to-emerald-500',
      );
      expect(css).toContain('radial-gradient');
      expect(css).toContain('conic-gradient');
      expect(css).toContain(BG_LAYER);
    ...
    test('bg-conic + border-linear', async () => {
      const css = await compile(
        'bg-conic from-blue-500 via-green-500 to-purple-500 border-linear-to-r border-from-amber-400 border-to-emerald-500',
      );
      expect(css).toContain('conic-gradient');
      expect(css).toContain('linear-gradient');
      expect(css).toContain(BG_LAYER);
    ...
    test('bg-conic + border-radial', async () => {
      const css = await compile(
        'bg-conic from-blue-500 via-green-500 to-purple-500 border-radial border-from-amber-400 border-to-emerald-500',
      );
      expect(css).toContain('conic-gradient');
      expect(css).toContain(BG_LAYER);
    });
    ...
    test('bg-linear + border-radial', async () => {
      const css = await compile(
        'bg-linear-to-r from-blue-500 to-purple-500 border-radial border-from-amber-400 border-to-emerald-500',
      );
      expect(css).toContain(BG_LAYER);
    });
    ...
    test('bg-linear + border-conic', async () => {
      const css = await compile(
        'bg-linear-to-r from-blue-500 to-purple-500 border-conic-0 border-from-amber-400 border-via-rose-500 border-to-emerald-500',
      );
      expect(css).toContain('conic-gradient');
      expect(css).toContain(BG_LAYER);
    });
...
describe.each(suiteScenarios('border-gradient'))(
  ...
  ({ compile }) => {
    test('oklch values', async () => {
      const css = await compile(
        'bg-slate-800 border-linear-to-r border-from-[oklch(0.6_0.25_330)] border-to-[oklch(0.7_0.2_200)]',
      );
    ...
    test('hsl values', async () => {
      const css = await compile(
        'bg-slate-800 border-linear-to-r border-from-[hsl(330,80%,50%)] border-to-[hsl(200,80%,50%)]',
      );
    ...
    test('rgb values', async () => {
      const css = await compile(
        'bg-slate-800 border-linear-to-r border-from-[rgb(255,100,50)] border-to-[rgb(0,180,216)]',
      );
...
describe.each(suiteScenarios('border-gradient'))(
  ...
  ({ compile }) => {
    ...
    test('border-conic without bg', async () => {
      const css = await compile(
        'border-conic-0 border-from-rose-500 border-via-yellow-400 border-to-cyan-500',
      );
      expect(css).toContain('conic-gradient');
      expect(css).toContain(BG_LAYER);
    });
    ...
    test('border-spin without bg', async () => {
      const css = await compile(
        'border-conic-0 border-spin border-from-rose-500 border-to-cyan-500',
      );
      expect(css).toContain('animation: border-spin');
      expect(css).toContain(BG_LAYER);
    });
