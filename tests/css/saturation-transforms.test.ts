 ...
 */
const STABLE_OKLCH =
  'oklch(from var(--tw-jib--background-color-after-hue-rotate, var(--tw-jib--background-color-source)) l calc(c * (1 - max(var(--tw-jib--background-saturation--amount), calc(0 - var(--tw-jib--background-saturation--amount)))) + max(0, var(--tw-jib--background-saturation--amount)) * 0.4) h / alpha)';
...
describe('stable path (relative color syntax)', () => {
  ...
  describe('desaturate — all 17 color spaces', () => {
    ...
    test('bg-desaturate-20/color-mix (inherit fallback)', async () => {
      ...
      expect(css).toContain('--tw-jib--background-color-after-saturation: inherit');
      expect(css).toContain('--tw-jib--background-saturation--amount: calc(20 * -0.01)');
    });
  });
...
describe('experimental path (@function + @supports)', () => {
  describe('desaturate — default amounts', () => {
    test.each([0, 5, 10, 20, 50, 75, 100])(
      ...
      async (amount) => {
        ...
        expect(css).toContain(`--tw-jib--saturation(var(--tw-jib--background-color-after-hue-rotate, var(--tw-jib--background-color-source)), calc(${amount} * -1))`);
      },
    );
  });
  ...
  describe('saturate — default amounts', () => {
    test.each([0, 5, 10, 20, 50, 75, 100])(
      ...
      async (amount) => {
        ...
        expect(css).toContain(SUPPORTS_FUNCTION);
        expect(css).toContain(`--tw-jib--saturation(var(--tw-jib--background-color-after-hue-rotate, var(--tw-jib--background-color-source)), ${amount})`);
      },
    );
  });
  ...
  describe('base color range', () => {
    test.each(DARKEN_PALETTE)('bg-%s bg-desaturate-20', async (color, marker) => {
      ...
      expect(css).toContain(SUPPORTS_FUNCTION);
      expect(css).toContain('--tw-jib--saturation(var(--tw-jib--background-color-after-hue-rotate, var(--tw-jib--background-color-source)), calc(20 * -1))');
      expect(css).toContain(marker);
    });
    ...
    test.each(LIGHTEN_PALETTE)('bg-%s bg-saturate-20', async (color, marker) => {
      ...
      expect(css).toContain(SUPPORTS_FUNCTION);
      expect(css).toContain('--tw-jib--saturation(var(--tw-jib--background-color-after-hue-rotate, var(--tw-jib--background-color-source)), 20)');
      expect(css).toContain(marker);
    });
  });
