...
test.describe('border spin', () => {
  ...
  /* The animation wins on cascade origin, so a frozen angle means it lost. */
  test('the animated angle beats the angle its conic border declares', async ({ page }) => {
    // Arrange
    await gotoPage(page);
  ...
  test('motion-safe:border-spin stops for a reduced-motion reader', async ({ page }) => {
    ...
    await gotoPage(page);
    const element = page.locator('[data-test="border-spin-motion-safe"]');
    // Act
    const spinning = await element.evaluate((node) => getComputedStyle(node).animationName);
    await page.emulateMedia({ reducedMotion: 'reduce' });
    const stilled = await element.evaluate((node) => getComputedStyle(node).animationName);
    // Assert
    expect(spinning).toBe('border-spin');
...
test.describe('state variants', () => {
  test('hover:bg changes background on hover', async ({ page }) => {
    await gotoPage(page);
